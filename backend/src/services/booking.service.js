...
const { getRoomById } = require('./room.service');
const { createInvoiceFile, formatMoney } = require('./invoice.service');
...
function servicesToText(services) {
  ...
  return services
    .map((service) => `${service.title} x${service.quantity} (${formatMoney(service.price * service.quantity)})`)
    .join(', ');
}

async function createBooking({ user, payload }) {
  if (!user?.email_verified) {
    throw createError(403, 'Vui long xac minh email truoc khi dat phong.');
  }

  const room = await getRoomById(payload.roomId);

  if (!room) {
    throw createError(404, 'Khong tim thay phong.');
  }
  ...
  const selectedServices = normalizeServices(payload.services);
  ...
  const servicesText = servicesToText(selectedServices);
  ...
  try {
    ...
    const invoice = await createInvoiceFile({
      booking,
      room,
      user,
      servicesText,
...
async function sendBookingConfirmationEmail({ user, room, booking, servicesText, invoicePath, invoiceCode }) {
  await sendMail({
    to: user.email,
    ...
    html: `
      <div style="font-family:Arial,sans-serif;line-height:1.7;color:#0f172a">
        ...
        <p>Tong tien: <strong>${formatMoney(booking.total_price)}</strong></p>
        <p>Hoa don: <strong>${invoiceCode}</strong></p>
      </div>
    `,
    text: `Dat phong thanh cong ${booking.booking_code}. Tong tien ${formatMoney(booking.total_price)}.`,
    attachments: [
      {
