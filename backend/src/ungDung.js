// Chá»©c nÄƒng: Láº¯p middleware, route public, route user vÃ  route admin.
// Bá»™ láº¯p route vÃ  middleware cho backend.
// File nÃ y gáº¯n middleware, route cÃ´ng khai, route khÃ¡ch hÃ ng vÃ  route quáº£n trá»‹.
...
ungDung.post(
  ...
  async (req, res) => {
    try {
      ...
      res.status(201).json({
        message: "ÄÃ¡nh giÃ¡ phÃ²ng thÃ nh cÃ´ng",
        data,
      });
    } catch (error) {
      res.status(error.status || 500).json({
        message: error.message || "KhÃ´ng thá»ƒ gá»­i Ä‘Ã¡nh giÃ¡ phÃ²ng.",
      });
    }
  },
);
