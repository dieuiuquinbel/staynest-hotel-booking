...
const LichSu = lazy(() => import('./pages/LichSu'));
...
function NutLienHeNoi() {
  return (
    <a
      href="https://zalo.me/0345583772"
      target="_blank"
      rel="noreferrer"
      className="fixed bottom-6 right-5 z-50 rounded-full bg-brand-600 px-5 py-3 text-sm font-black text-white shadow-lg shadow-brand-600/30 transition hover:-translate-y-0.5 hover:bg-brand-700 focus:outline-none focus:ring-4 focus:ring-sky-200"
    >
      Liên hệ ngay
    </a>
  );
...
function NutCuonTrangNoi() {
  ...
  return (
    <button
      ...
      className="fixed bottom-[86px] right-6 z-50 flex h-11 w-11 items-center justify-center rounded-full border border-sky-100 bg-white/95 text-[#0f5d82] shadow-md shadow-slate-900/10 backdrop-blur transition hover:border-sky-200 hover:bg-sky-50 focus:outline-none focus:ring-4 focus:ring-sky-100"
    >
      {icon}
    </button>
  );
