        ...
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
          <div className="grid overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_24px_65px_-54px_rgba(15,23,42,0.45)] lg:grid-cols-[minmax(0,1.04fr)_minmax(340px,0.96fr)]">
            <div className="review-stream-panel relative h-full overflow-hidden bg-slate-950 p-5 text-white sm:p-6">
              ...
              <div className="review-stack-stage relative z-10 mt-5 h-[380px] overflow-hidden sm:h-[400px] lg:h-[392px]">
                <div className="review-stream-track grid gap-3">
                  {[...DANH_GIA, ...DANH_GIA].map((review, index) => (
                    <article
                      key={`${review.name}-${index}`}
                      className={`review-bubble ${index % 3 === 0 ? 'review-bubble-left' : index % 3 === 1 ? 'review-bubble-right' : 'review-bubble-mid'}`}
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <p className="text-base font-black text-white">{review.name}</p>
                          <p className="mt-0.5 text-xs font-bold text-white/62">{review.trip}</p>
                        </div>
                        <span className="rounded-lg bg-brand-600 px-3 py-2 text-lg font-black text-white">{review.score}</span>
                      </div>
                      <p className="mt-3 inline-flex rounded-full bg-white/10 px-3 py-1 text-xs font-bold text-white/82">
                        {review.tag}
                      </p>
                      <p className="mt-3 text-sm leading-7 text-white/86">{review.quote}</p>
                    </article>
                  ))}
                </div>
              </div>
