...
import { loginUser, registerUser, resendEmailOtp, verifyEmailOtp } from '../services/authApi';
...
function AuthPage() {
  ...
  const resendMutation = useMutation({ mutationFn: resendEmailOtp });
  ...
  const handleLoginSubmit = async (event) => {
    ...
    try {
      ...
      setLoginError(readErrorMessage(error, 'Không thể đăng nhập. Vui lòng kiểm tra thông tin.'));
    }
  };
  ...
  const handleVerifySubmit = async (event) => {
    ...
    try {
      ...
      setVerifyError(readErrorMessage(error, 'Không thể xác minh OTP. Vui lòng thử lại.'));
    }
  };
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      setVerifyError(readErrorMessage(error, 'Không thể gửi lại OTP.'));
    }
  };
            ...
            <div className="auth-form-stage">
              {isVerifying ? (
                <form onSubmit={handleVerifySubmit} className="auth-form-panel mt-8 grid gap-5">
                  <Notice tone="info">OTP có hiệu lực trong 10 phút. Kiểm tra cả hộp thư spam nếu chưa thấy email.</Notice>
                  {verification.devOtp ? <Notice tone="warning">SMTP chưa cấu hình, dùng mã dev OTP: {verification.devOtp}</Notice> : null}
                  <FormField
                    label="Mã OTP"
                    type="text"
                    inputMode="numeric"
                  ...
                  />
                  {verifyError ? <Notice>{verifyError}</Notice> : null}
                ...
                <form onSubmit={handleLoginSubmit} className="auth-form-panel mt-8 grid gap-5">
                  ...
                  />
                  {loginError ? <Notice>{loginError}</Notice> : null}
                  <button
                    type="submit"
