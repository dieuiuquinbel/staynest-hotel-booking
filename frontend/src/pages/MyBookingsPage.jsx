...
function MyBookingsPage() {
  ...
  const bookings = readMyBookings(user?.id || user?.email);
        ...
        {filteredBookings.length ? (
          <div className="mt-6 grid gap-5">
            {filteredBookings.map((booking) => (
              <article
                key={booking.id}
                className="subtle-card grid overflow-hidden lg:grid-cols-[240px_minmax(0,1fr)_280px] xl:grid-cols-[260px_minmax(0,1fr)_310px]"
              >
                ...
                <div className="flex min-w-0 flex-col justify-between gap-4 border-t border-sky-100 bg-sky-50/60 p-5 lg:border-l lg:border-t-0">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 break-words text-sm leading-6 text-slate-500">{booking.guestEmail}</p>
                  </div>

                  <div className="grid gap-3">
                    <Link
                      to={`/rooms/${booking.roomId}`}
                      className="flex min-h-12 items-center justify-center rounded-xl border border-sky-200 bg-white px-4 py-3 text-center text-sm font-bold text-brand-700 transition hover:bg-sky-50"
                    >
                      Xem khách sạn
                    </Link>
                    <button
                    ...
                    <button
                      ...
                      className="min-h-12 rounded-xl bg-slate-950 px-4 py-3 text-sm font-bold text-white transition hover:bg-brand-700 disabled:cursor-not-allowed disabled:bg-slate-300"
                    >
                      Hủy giữ chỗ
                    </button>
                  </div>
