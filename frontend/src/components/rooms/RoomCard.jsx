...
function RoomCard({ room, layout = 'horizontal' }) {
    ...
    return (
      <article className="subtle-card flex h-full flex-col overflow-hidden">
        <div className="relative h-56 overflow-hidden bg-sky-100">
          ...
          <span className="absolute right-3 top-3 rounded-xl bg-white px-3 py-2 text-sm font-black text-brand-700 shadow-sm">
        ...
        <div className="flex flex-1 flex-col p-5">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <h3 className="mt-2 line-clamp-2 text-xl font-black leading-6 tracking-tight text-slate-950">{room.hotel_name}</h3>
          ...
          <div className="mt-auto flex items-end justify-between gap-3 pt-5">
            <div>
              ...
              <p className="mt-1 text-xl font-black text-slate-950">{formatCurrency(room.price_per_night)}</p>
              <p className="text-xs text-slate-500">Tối đa {room.max_guests || 2} khách</p>
            </div>
            <Link
              to={`/rooms/${room.id}`}
  ...
  return (
    <article className="subtle-card grid overflow-hidden bg-white lg:grid-cols-[300px_minmax(0,1fr)_210px]">
      <div className="relative min-h-[250px] overflow-hidden bg-sky-100">
        <img
          src={room.image_url}
          alt={`${room.hotel_name} ${room.room_name}`}
          loading="lazy"
      ...
      <div className="min-w-0 p-5 lg:p-6">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {room.free_cancellation ? (
            <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">Hủy miễn phí</span>
          ) : null}
          {room.breakfast_included ? (
            <span className="rounded-full bg-sky-50 px-3 py-1 text-xs font-bold text-sky-700">Bữa sáng</span>
          ) : null}
          <FavoriteButton room={room} />
        </div>

        <h3 className="mt-4 text-2xl font-black tracking-tight text-slate-950">{room.hotel_name}</h3>
        <p className="mt-1 text-sm font-bold text-slate-600">{room.room_name}</p>
        <p className="mt-2 line-clamp-1 text-sm font-medium text-slate-500">{room.address}</p>
        <p className="mt-4 line-clamp-2 text-sm leading-7 text-slate-600">{room.description}</p>

        <div className="mt-4 flex flex-wrap gap-2">
          {amenities.map((amenity) => (
            <span key={`${room.id}-${amenity}`} className="rounded-full bg-sky-50 px-3 py-1.5 text-xs font-bold text-slate-600">
              {formatAmenity(amenity)}
            </span>
          ))}
        </div>
      ...
      <div className="flex flex-col justify-between border-t border-sky-100 bg-gradient-to-b from-sky-50 to-white p-5 lg:border-l lg:border-t-0">
        <div className="flex items-start justify-between gap-4 lg:block">
          <div className="rounded-2xl bg-brand-600 px-4 py-3 text-white shadow-sm shadow-brand-500/20">
            <p className="text-xs font-semibold text-sky-100">Đánh giá</p>
            <p className="mt-1 text-3xl font-black">{formatRating(room.rating_avg)}</p>
            <p className="mt-1 text-xs text-sky-100">{room.total_reviews} lượt đánh giá</p>
          </div>
          <div className="text-right lg:mt-5 lg:text-left">
            <p className="text-xs font-semibold text-slate-500">Giá mỗi đêm</p>
            <p className="mt-1 text-2xl font-black tracking-tight text-slate-950">{formatCurrency(room.price_per_night)}</p>
            <p className="mt-1 text-sm text-slate-500">Tối đa {room.max_guests || 2} khách</p>
          </div>
        </div>

        <div className="mt-5 grid gap-3">
          <p className={`text-sm font-extrabold leading-6 ${availability.textClass}`}>{availability.description(room.inventory_count)}</p>
          <Link
            to={`/rooms/${room.id}`}
